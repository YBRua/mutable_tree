...
from ..nodes import (ArrayAccess, ArrayCreationExpression, ArrayExpression,
                     ...
                     CastExpression, CommaExpression, CompoundLiteralExpression,
                     DeleteExpression, FieldAccess, Identifier, InstanceofExpression,
                     Literal, NewExpression, ParenthesizedExpression, PointerExpression,
                     QualifiedIdentifier, SizeofExpression, TernaryExpression,
...
from ..nodes import (AssertStatement, BlockStatement, BreakStatement, ContinueStatement,
                     ...
                     ForStatement, GotoStatement, IfStatement, LabeledStatement,
                     LambdaExpression, ReturnStatement, SwitchStatement,
                     SynchronizedStatement, ThrowStatement, TryStatement,
                     TryWithResourcesStatement, WhileStatement, YieldStatement)
from ..nodes import (DimensionSpecifier, Dimensions, TypeIdentifier, TypeParameterList,
                     TypeParameter)
...
class BaseStringifier:
    ...
    def stringify_AssignmentExpression(self, node: AssignmentExpression) -> str:
        return f'{self.stringify(node.left)} {node.op.value} {self.stringify(node.right)}'
    ...
    def stringify_CompoundLiteralExpression(self, node: CompoundLiteralExpression) -> str:
    ...
    def stringify_FieldAccess(self, node: FieldAccess) -> str:
        return f'{self.stringify(node.object)}{node.op.value}{self.stringify(node.field)}'
    ...
    def stringify_TernaryExpression(self, node: TernaryExpression) -> str:
        return (f'{self.stringify(node.condition)} ? '
    ...
    def stringify_ForStatement(self, node: ForStatement) -> str:
        if node.init is None:
            ...
            init_str = ', '.join(
                self.stringify(init) for init in node.init.get_children()) + ';'
    ...
    def stringify_TryWithResourcesStatement(self, node: TryWithResourcesStatement) -> str:
        resource_str = '; '.join(
    ...
    def stringify_TypeParameterDeclaration(self, node: TypeParameterDeclaration) -> str:
        return f'{self.stringify(node.typename_opt.value)} {self.stringify(node.type_id)}'
