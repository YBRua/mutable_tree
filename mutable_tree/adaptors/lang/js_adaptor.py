...
from ...nodes import (ArrayAccess, ArrayExpression, AssignmentExpression,
                      BinaryExpression, CallExpression, FieldAccess, Identifier, Literal,
                      NewExpression, TernaryExpression, ThisExpression, UnaryExpression,
                      UpdateExpression, ParenthesizedExpression, ExpressionList,
                      LambdaExpression, SpreadElement, AwaitExpression, CommaExpression)
from ...nodes import (BlockStatement, BreakStatement, ContinueStatement, DoStatement,
                      ...
                      FinallyClause, TryHandlers, WhileStatement, YieldStatement,
                      WithStatement)
...
def convert_for_in_stmt(node: tree_sitter.Node) -> ForInStatement:
    ...
    return node_factory.create_for_in_stmt(decl_ty, left, right, body, forin_type,
                                           is_async)
...
def convert_try_stmt(node: tree_sitter.Node) -> TryStatement:
    ...
    finalizer = convert_try_finalizer(
        finalizer_node) if finalizer_node is not None else None
