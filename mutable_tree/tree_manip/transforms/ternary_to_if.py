...
from mutable_tree.nodes import (Expression, ExpressionStatement, AssignmentExpression,
                                TernaryExpression)
...
class TernaryToIfVisitor(TransformingVisitor):
    ...
    def _wrap_assignment_block(self, lhs: Expression, rhs: Expression):
        ...
        return node_factory.create_block_stmt(node_factory.create_statement_list([stmt]))

    def visit_ExpressionStatement(self,
        ...
        new_node = node_factory.create_if_stmt(condition, consequence_block,
                                               alternative_block)
